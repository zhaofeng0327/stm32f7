...
typedef union {
	...
	#endif	// if __CORTEX_A == 5 || defined(DOXYGEN)
	...
	#endif	// if __CORTEX_A == 7 || defined(DOXYGEN)
	...
	#endif		// if __CORTEX_A == 9 || defined(DOXYGEN)
	uint32_t w;	/*!< \brief Type      used for word access */
} ACTLR_Type;
...
#endif																			// if (__GIC_PRESENT == 1U) || defined(DOXYGEN)
...
__STATIC_FORCEINLINE void __L1C_MaintainDCacheSetWay(uint32_t level, uint32_t maint)
{
	...
	for (int32_t way = num_ways - 1; way >= 0; way--) {
		for (int32_t set = num_sets - 1; set >= 0; set--) {
			...
			switch (maint) {
				case 0U: __set_DCISW(Dummy);
					break;
				case 1U: __set_DCCSW(Dummy);
					break;
				default: __set_DCCISW(Dummy);
					break;
			}
		}
	}
...
#endif	// if (__CORTEX_A == 7U) || defined(DOXYGEN)
...
__STATIC_INLINE int MMU_APSection(uint32_t *descriptor_l1, mmu_access_Type user, mmu_access_Type priv, uint32_t afe)
{
	...
	if (afe == 0) {	// full access
		...
		else if ((priv == RW) && (user == READ)) ap = 0x2; else if ((priv == RW) && (user == RW)) ap = 0x3;
		else if ((priv == READ) && (user == NO_ACCESS)) ap = 0x5; else if ((priv == READ) && (user == READ)) ap = 0x7;
	} else {// Simplified access
		if ((priv == RW) && (user == NO_ACCESS)) ap = 0x1; else if ((priv == RW) && (user == RW)) ap = 0x3;
		else if ((priv == READ) && (user == NO_ACCESS)) ap = 0x5; else if ((priv == READ) && (user == READ)) ap = 0x7;
	}
...
__STATIC_INLINE int MMU_APPage(uint32_t *descriptor_l2, mmu_access_Type user, mmu_access_Type priv, uint32_t afe)
{
	...
	if (afe == 0) {	// full access
		...
		else if ((priv == READ) && (user == NO_ACCESS)) ap = 0x5; else if ((priv == READ) && (user == READ)) ap = 0x6;
	} else {// Simplified access
		if ((priv == RW) && (user == NO_ACCESS)) ap = 0x1; else if ((priv == RW) && (user == RW)) ap = 0x3;
		else if ((priv == READ) && (user == NO_ACCESS)) ap = 0x5; else if ((priv == READ) && (user == READ)) ap = 0x7;
	}
