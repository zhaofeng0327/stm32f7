...
static char recv_data_dispatch(unsigned char slave, void *pt, unsigned int len)
{
	...
	switch (type) {
		...
		case RES_BATTERY_INFO:
		{
			...
			dzlog_debug("get battery info code %d, sn %s temp %d vol %d cur %d full %d rem %d cnt %d\n", res->code,
			res->sn,
			res->temperature,	//温度
			res->voltage,	//电压
			res->current,	//电流
...
static void *thread_send_queue(void *p)
{
	...
	while (1) {
		...
		if (lens >= 0) {
			...
			char rq = head->type;
			sleep_count = 0;
			if (!is_req_type_valid(rq) && !is_res_type_valid(rq)) {
